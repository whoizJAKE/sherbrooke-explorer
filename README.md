...
Map data © OpenStreetMap contributors, available under the Open Database License. The same credit is shown in the game. Elevation is the terrarium tiles already packed in `public/world.pack`. Surface textures, tree cards, and normal maps are generated by `node scripts/make-textures.mjs` and are original to this project. They are not sampled from a game, a street-level photo, or a commercial map.
